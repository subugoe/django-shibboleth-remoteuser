...
settings.AUTHENTICATION_BACKENDS += ("shibboleth.backends.ShibbolethRemoteUserBackend",)
...
from shibboleth import apps  # noqa: E402
...
class TestShibbolethRemoteUserBackend(TestCase):
    def _get_valid_shib_meta(self, location="/"):
        ...
        shib_meta, error = middleware.ShibbolethRemoteUserMiddleware.parse_attributes(
            test_request
        )
        self.assertFalse(
            error, "Generating shibboleth attribute mapping contains errors"
        )
        return shib_meta
    ...
    def test_create_unknown_user_true(self):
        ...
        user = auth.authenticate(
            remote_user="sampledeveloper@school.edu", shib_meta=shib_meta
        )
...
class TestShibbolethParseAttributes(TestCase):
    ...
    def test_mutated_attribute(self):
        ...
        self.assertEqual(
            shib_meta["shortened_name"], SAMPLE_HEADERS["Shibboleth-displayName"][:5]
        )
...
class TestShibbolethGroupAssignment(TestCase):
    ...
    def test_group_creation_list(self):
        ...
        with self.settings(
            SHIBBOLETH_GROUP_ATTRIBUTES=[
                "Shibboleth-affiliation",
                "Shibboleth-isMemberOf",
            ]
        ):
            reload(apps)
            reload(middleware)
    ...
    def test_group_creation_list_comma(self):
        ...
        with self.settings(
            SHIBBOLETH_GROUP_ATTRIBUTES=["Shibboleth-isMemberOf-multi-delimiter"],
    ...
    def test_group_creation_list_mixed_comma_semicolon(self):
        ...
        with self.settings(
            SHIBBOLETH_GROUP_ATTRIBUTES=[
                "Shibboleth-affiliation",
                "Shibboleth-isMemberOf-multi-delimiter",
            ],
            SHIBBOLETH_GROUP_DELIMITERS=[",", ";"],
        ):
            reload(apps)
    ...
    def test_empty_group_attribute(self):
        ...
        with self.settings(
            SHIBBOLETH_GROUP_ATTRIBUTES=["Shibboleth-not-existing-attribute"]
        ):
            reload(apps)
            reload(middleware)
            self.client.get("/", **SAMPLE_HEADERS)
...
class LogoutTest(TestCase):
    def test_logout(self):
        ...
        self.assertEqual(
            logout["Location"], "https://sso.school.edu/logout?next=http://school.edu/"
        )
